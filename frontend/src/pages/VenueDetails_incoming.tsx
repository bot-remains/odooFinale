...
const VenueDetails = () => {
  ...
  const displayVenue = venue
    ? {
        ...
        facilities: (venue.amenities || []).map((name) => ({
          name,
          icon: Shield,
        })),
      }
      ...
      <div className="container py-10">
        <div className="text-center py-20">
          ...
          <p className="text-muted-foreground mb-4">
            Unable to load venue details. The venue may not exist or the server
            is unavailable.
          </p>
          <Button asChild>
  ...
  return (
    <>
      <SEO
        title={
          displayVenue
            ? `${displayVenue.name} – QuickCourt`
            : "Venue Details – QuickCourt"
        }
        description={
          displayVenue
            ? `Book ${displayVenue.name}. ${
                displayVenue.sports?.join(", ") || "Sports venue"
              }. Starting ₹${displayVenue.price}/hr in ${
                displayVenue.location
              }.`
            : "Venue booking details on QuickCourt"
        }
      />
      ...
      <section className="container py-10 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-2">
                  {renderStars(
                    typeof displayVenue.rating === "string"
                      ? parseFloat(displayVenue.rating)
                      : displayVenue.rating || 0
                  )}
                  <span className="font-medium text-foreground">
                    {typeof displayVenue.rating === "string"
                      ? parseFloat(displayVenue.rating).toFixed(1)
                      : displayVenue.rating?.toFixed(1) || "0.0"}
                  </span>
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-8">
          ...
          <div className="space-y-8">
            ...
            {/* Available Courts */}
            {displayVenue.courts && displayVenue.courts.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>Available Courts</CardTitle>
                  <p className="text-sm text-muted-foreground">
                    Select a court to view pricing and book your session
                  </p>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-4">
                    {displayVenue.courts.map((court: any) => (
                      <div
                        key={court.id}
                        className="border rounded-lg p-4 hover:border-blue-300 transition-colors cursor-pointer"
                        onClick={() => {
                          // Set selected court and show booking section
                              ...
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                {court.amenities &&
                                  court.amenities.length > 0 && (
                                    <span>
                                      • {court.amenities.slice(0, 2).join(", ")}
                                    </span>
                                  )}
                              </div>
                            </div>
                ...
                {selectedCourt ? (
                  <>
                    ...
                    <Button className="w-full" size="lg" asChild>
                      <Link
                        to={`/booking/${displayVenue.id}?courtId=${selectedCourt.id}`}
                      >
                        Book This Court
                      </Link>
                    </Button>
                  ...
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-2xl font-bold">
                        ₹{displayVenue.price > 0 ? displayVenue.price : "N/A"}
                        /hr
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Clock className="h-4 w-4" />
                      Select a court to see exact pricing
                    </div>
                    {displayVenue.courts && displayVenue.courts.length > 0 ? (
                      <Alert>
                        <AlertTriangle className="h-4 w-4" />
                        <AlertDescription>
                          Please select a court from the list below to proceed
                          with booking.
                        </AlertDescription>
                      </Alert>
                    ) : (
                      <Button className="w-full" size="lg" asChild>
                        <Link to={`/booking/${displayVenue.id}`}>Book Now</Link>
                      </Button>
                    )}
                  </>
                )}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {displayVenue.totalReviews || reviews.length}
                  </span>
                </div>
                {displayVenue.sports && displayVenue.sports.length > 0 && (
                  <div className="flex items-center justify-between">
            ...
            <CardContent>
              <form onSubmit={handleReportSubmit} className="space-y-4">
                ...
                <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    Reports are reviewed by our team. False reports may result
